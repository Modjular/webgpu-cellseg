 ...
 *
 * `mode` selects the surrounding arithmetic, which genuinely differs between the models —
 * it is not a flag that could be folded into a uniform:
 *
 *   "bn"     Cellpose. BN scale/shift and relu are applied to the *input* as it is
 *            staged (cellpose's batchconv is BN → relu → conv), plus an optional skip
 *            added pre-BN and an optional residual added post-conv.
 *   "plain"  StarDist and InstanSeg. No BN, and relu is applied to the *output*.
 *
 * Everything that determines throughput — the tiling, the register block, the staging,
 * the inner loop — is shared, so all three models are tuned by the one sweep in
 * tools/convbench.mjs.
 */
export function convWGSL(K, rby = RBY, rbx = RBX, blk = BLK, cb = CB, mode = "bn") {
  const bn = mode === "bn";
  if (mode !== "bn" && mode !== "plain") throw new Error(`convWGSL: unknown mode ${mode}`);
  ...
  // The one place the two modes differ after staging: Cellpose adds its residual here,
  // StarDist/InstanSeg apply relu here instead (their conv is conv → relu, not BN → relu
  // → conv).
  const epilogue = bn
    ? "if (p.useResid == 1u) { t = t + resid[oi]; }"
    : "if (p.useRelu == 1u) { t = max(t, 0.0); }";
  ...
  for (let py = 0; py < rby; py++) {
    ...
    if (ox2 < p.W && oy2 < p.H) {
      let o = oy2 * p.W + ox2;
${chans.map((c) => `      if (coBase + ${c}u < p.Cout) { let oi = (coBase + ${c}u) * HW + o; var t = ${acc(c, p)}; ${epilogue} outp[oi] = t; }`).join("\n")}
    }
  }`);
    }
  }
  ...
  return /* wgsl */`
// generated by src/conv-kernel.js — mode=${mode}, K=${K}, block ${rby}x${rbx}, BLK=${blk}, CB=${cb}
${bn ? `struct P { H:u32, W:u32, Cin:u32, Cout:u32, K:u32, pad:u32, useRelu:u32, useAdd:u32,
           useResid:u32, _p0:u32, _p1:u32, _p2:u32 };` : `struct P { H:u32, W:u32, Cin:u32, Cout:u32, K:u32, pad:u32, useRelu:u32, _p:u32 };`}
@group(0) @binding(0) var<uniform> p: P;
@group(0) @binding(1) var<storage,read>       inp:   array<f32>;
@group(0) @binding(2) var<storage,read>       w:     array<f32>;
@group(0) @binding(3) var<storage,read>       b:     array<f32>;
${bn ? `@group(0) @binding(4) var<storage,read>       scale: array<f32>;
...
${chans.flatMap((c) => pixels.map((p) => `  var ${acc(c, p)} = bc${c};`)).join("\n")}
  ...
  for (var ci = 0u; ci < p.Cin; ci = ci + ${cb}u) {
    // Cooperative load of the staged activation tiles.${bn ? `
    // The optional skip add, the BN scale/shift and the optional relu are applied on the
    // way in — the fusion that makes the elementwise operators free here (Phase 0
    // measured all non-conv work at 0.19% of GPU time).` : ""}
    for (var i = lt; i < ${cb * TWxy}u; i = i + TS * TS) {
      ...
      if (ci + s < p.Cin && gy >= 0 && gy < i32(p.H) && gx >= 0 && gx < i32(p.W)) {
        ...
        v = inp[idx];
${bn ? `        if (p.useAdd == 1u) { v = v + addv[idx]; }
